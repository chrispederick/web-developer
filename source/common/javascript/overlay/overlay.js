...
WebDeveloper.Overlay.addFeatureOnTab = function(featureItem, tab, scriptFile, scriptCode, args)
{
  WebDeveloper.Overlay.addScriptsToTab(tab, scriptFile, scriptCode, args, null);
};
...
WebDeveloper.Overlay.addScriptsToTab = function(tab, scriptFile, scriptCode, args, callback)
{
  chrome.scripting.executeScript({ target: { tabId: tab.id }, files: [scriptFile] }).then(() =>
...
WebDeveloper.Overlay.displayConfirmation = function(title, message, buttonText, buttonIcon, callback)
{
  ...
  if(buttonIcon)
  {
    buttonHTML = '<svg class="bi text-success"><use xlink:href="svg/bootstrap-icons.svg#' + buttonIcon + '"></svg> ' + buttonText;
  }
...
WebDeveloper.Overlay.displayNotification = function(message, type)
{
  ...
  notification.querySelector("span").replaceChildren(message);
...
WebDeveloper.Overlay.initialize = function()
{
  ...
  WebDeveloper.Storage.getItem("menu", function(menu)
  {
    ...
    if(!menu)
    {
      menu = document.querySelector(".nav-tabs > li:visible:first").getAttribute("id");
    }
...
WebDeveloper.Overlay.toggleFeatureOnTab = function(featureItem, tab, scriptFile, scriptCode, args, closeOverlay)
{
  var feature = featureItem.getAttribute("id");

  WebDeveloper.Overlay.addScriptsToTab(tab, scriptFile, scriptCode, args, function()
  {
    WebDeveloper.Storage.toggleFeatureOnTab(feature, tab);
